...
const CITY = {Berlin:{n:81,intent:7.40,spend:19.98,share:18,cagr:9,segments:{'Urban Wellness Professionals':27,'On-the-go Commuters':19,'Fitness & Gym-Goers':14,'Students & Budget-Conscious':21},channels:{'Retail / Grocery':39,'Gym & Office':23,'DTC Online':19},scores:{market:92,access:88,competition:72,visibility:86,finance:79,risk:68},call:'Best first learning market',why:'Largest named-city sample, strong intent, broad segment access and enough channel variety to learn fast.'},Munich:{n:57,intent:7.30,spend:19.89,share:15,cagr:9,scores:{market:82,access:76,competition:78,visibility:72,finance:76,risk:74},call:'Efficient premium test',why:'Strong growth and a cleaner premium proposition, but a smaller learning base than Berlin.'},Frankfurt:{n:32,intent:7.18,spend:20.93,share:8,cagr:7,scores:{market:65,access:70,competition:81,visibility:68,finance:83,risk:78},call:'Margin-led alternative',why:'Highest spend signal and attractive economics, offset by the smallest survey base and lower scale.'},Cologne:{n:45,intent:7.04,spend:18.10,share:9,cagr:7,scores:{market:61,access:67,competition:74,visibility:70,finance:60,risk:62},call:'Controlled challenger',why:'Useful as a lower-cost test, but weaker intent and spend make it less compelling as the first signal.'}};
...
function renderCity(tab='case'){const b=CITY.Berlin,other=Object.entries(CITY).filter(([k])=>k!=='Berlin');let html='';if(tab==='case')html=`<div class="city-content-grid"><div class="city-stat"><span>Survey signal</span><strong>#1</strong><small>Largest named city sample: ${b.n} respondents</small></div><div class="city-stat"><span>Purchase intent</span><strong>${b.intent.toFixed(2)} / 10</strong><small>Above Munich (7.30) and Cologne (7.04)</small></div><div class="city-stat"><span>Market tailwind</span><strong>${b.share}%</strong><small>Share of market · ${b.cagr}% regional CAGR</small></div></div><div class="city-source">Berlin is selected against named-city survey and market context benchmarks, not population alone. Source: Exhibits 1 &amp; 4.</div>`;if(tab==='segments')html=`<div class="city-content-grid"><div class="city-story"><span>Primary audience</span><h4>Urban Wellness Professionals</h4><p><strong>27 / ${b.n} respondents</strong> · the largest Berlin segment. Lead with clean-label energy, premium cues, and DTC storytelling.</p></div><div class="city-story"><span>Conversion bridge</span><h4>On-the-go Commuters + Fitness</h4><p><strong>${b.segments['On-the-go Commuters']+b.segments['Fitness & Gym-Goers']} respondents</strong> · translate functional benefits into retail trial and Gym &amp; Office sampling.</p></div><div class="city-story"><span>Keep learning</span><h4>Students &amp; Budget-Conscious</h4><p><strong>${b.segments['Students & Budget-Conscious']} respondents</strong> · a meaningful price-sensitive cohort; test €2.19 before a premium step-up.</p></div></div><div class="city-source">Source: Exhibit 4 · Berlin segment counts from the German customer survey; names/emails are not used.</div>`;if(tab==='playbook')html=`<div class="city-content-grid"><div class="city-story"><span>01 · Trial engine</span><h4>Retail / Grocery first</h4><p>Berlin channel preferences show Retail / Grocery as the lead route (${b.channels['Retail / Grocery']} respondents), matching the existing €2.19 retail-led recommendation.</p></div><div class="city-story"><span>02 · Proof loop</span><h4>Gym &amp; Office support</h4><p>Use targeted sampling and office/gym partnerships to reach functional-beverage users without overcommitting to mass distribution.</p></div><div class="city-story"><span>03 · Margin layer</span><h4>DTC close behind</h4><p>Retarget trial with DTC subscriptions, where contribution is strongest; keep the first city launch measurable and reversible.</p></div></div><div class="city-source">Source: Exhibits 4, 9 &amp; 11 · consistent with the existing €2.19 price and channel mix.</div>`;$('cityPanelContent').innerHTML=html;}

function renderCityComparison(tab='case') { const cities=['Berlin','Munich','Frankfurt','Cologne']; const selected=tab==='case'?'Berlin':tab; const city=CITY[selected]||CITY.Berlin; const dims=[['market','Market potential'],['access','Customer access'],['competition','Competitive whitespace'],['visibility','Launch visibility'],['finance','Financial attractiveness'],['risk','Risk readiness']]; $('cityPanelContent').innerHTML=`<div class="city-map-grid"><div class="germany-map" aria-label="City comparison map"><span class="map-label">GERMANY / FIRST CITY DECISION</span><i class="city-pin berlin active" data-city="Berlin">BER</i><i class="city-pin munich" data-city="Munich">MUC</i><i class="city-pin frankfurt" data-city="Frankfurt">FRA</i><i class="city-pin cologne" data-city="Cologne">CGN</i><div class="map-route"></div></div><div class="city-compare"><span class="eyebrow">CITY SHORTLIST</span>${cities.map(name=>`<button class="city-choice ${name===selected?'selected':''}" data-city="${name}"><strong>${name}</strong><span>${CITY[name].call}</span><b>${Math.round(Object.values(CITY[name].scores).reduce((a,v)=>a+v,0)/6)}</b></button>`).join('')}</div></div><div class="city-decision"><div><span class="eyebrow">THE CALL · ${selected.toUpperCase()}</span><h4>${city.call}</h4><p>${city.why}</p><small>Aligned plan: €2.19 balanced price · Retail / Grocery trial · DTC retention · April launch window.</small></div><div class="city-dimensions">${dims.map(([key,label])=>`<div><span>${label}</span><div class="dimension-track"><i style="width:${city.scores[key]}%"></i></div><b>${city.scores[key]}</b></div>`).join('')}</div></div><div class="city-source">Sources: Exhibits 1 &amp; 4 · city survey, market context, segment and channel data. Scores are directional decision weights, not a forecast.</div>`; document.querySelectorAll('.city-choice,.city-pin').forEach(el=>el.addEventListener('click',()=>renderCityComparison(el.dataset.city))); }
renderCity = renderCityComparison;
const cityComparisonBase=renderCityComparison;renderCityComparison=function(tab='case'){if(['case','Berlin','Munich','Frankfurt','Cologne'].includes(tab))return cityComparisonBase(tab);cityComparisonBase('case');const panel=$('cityPanelContent');const lens=tab==='segments'?`<div class="city-lens"><span class="eyebrow">EVIDENCE LENS</span><h4>Who can make Berlin work?</h4><p><strong>27 Urban Wellness Professionals</strong> are the lead cohort; add 33 commuters and fitness users through retail trial and Gym &amp; Office sampling. The 21 students remain the price-sensitivity check.</p></div>`:`<div class="city-lens"><span class="eyebrow">LAUNCH PLAN LENS</span><h4>How should the first city learn?</h4><p>Start with Retail / Grocery for discovery, use Gym &amp; Office for proof, then move repeat demand into DTC. The CEO can vary price, month and mix above and watch this city ranking respond.</p></div>`;panel.insertAdjacentHTML('beforeend',lens);};
state.city='Berlin';
document.querySelectorAll('[data-page-tab]').forEach(button=>button.addEventListener('click',()=>{const target=button.dataset.pageTab;const pricing=document.querySelector('.pricing-panel'),visual=document.querySelector('.visual-grid'),metrics=document.querySelector('.metric-grid'),recommendation=document.querySelector('.recommendation-panel');if(target==='plan'){pricing.style.display='none';visual.style.display='none';metrics.style.display='grid';recommendation.style.display='grid';}if(target==='pricing'){pricing.style.display='block';visual.style.display='grid';metrics.style.display='grid';recommendation.style.display='none';}}));
document.querySelector('.pricing-panel').style.display='none';document.querySelector('.visual-grid').style.display='none';
function refreshCityContext(){const city=CITY[state.city]||CITY.Berlin;const month=Number($('launchMonth')?.value||4);const price=Number(state.price);const timing=TIMING[month-1];const priceFit=price===2.19?0:price===1.79?4:-5;const timingFit=month>=3&&month<=8?4:month<=2||month>=11?-5:0;const factor=priceFit+timingFit;const scores=Object.values(city.scores).map(v=>clamp(v+factor,40,96));const note=document.querySelector('.city-decision small');if(note)note.textContent=`Live plan: €${price.toFixed(2)} · ${timing.m} launch · ${state.mix.retail}% Retail / Grocery · ${state.mix.dtc}% DTC. Adjust the controls above to test the city decision.`;document.querySelectorAll('.city-dimensions i').forEach((bar,i)=>bar.style.width=`${scores[i]}%`);document.querySelectorAll('.city-dimensions b').forEach((el,i)=>el.textContent=scores[i]);}
document.addEventListener('click',(event)=>{const el=event.target.closest('.city-choice,.city-pin');if(el){state.city=el.dataset.city;setTimeout(refreshCityContext,0);}},true);
if($('launchMonth'))$('launchMonth').addEventListener('input',refreshCityContext);document.querySelectorAll('#priceOptions button').forEach(el=>el.addEventListener('click',refreshCityContext));document.querySelectorAll('#dtcRange,#retailRange,#gymRange').forEach(el=>el.addEventListener('input',refreshCityContext));

document.querySelectorAll('[data-page-tab]').forEach(button=>button.addEventListener('click',()=>{const target=button.dataset.pageTab;document.querySelectorAll('[data-page-tab]').forEach(tab=>tab.classList.toggle('active',tab===button));const hero=document.querySelector('.hero-grid'),main=document.querySelector('.main-stack'),timing=$('timingLab'),city=$('cityLab');hero.style.display=target==='timing'||target==='city'?'none':'';main.style.display=target==='timing'||target==='city'?'none':'';timing.style.display=target==='timing'?'grid':'none';city.style.display=target==='city'?'block':'none';if(target==='plan'||target==='pricing'){hero.style.display='';main.style.display='';window.scrollTo({top:document.querySelector('.page-tabs').offsetTop-20,behavior:'smooth'});}}));
document.querySelectorAll('[data-page-tab]').forEach(button=>{if(button.dataset.pageTab!=='plan'){const target=button.dataset.pageTab;const hero=document.querySelector('.hero-grid'),main=document.querySelector('.main-stack');if(target==='timing'||target==='city'){hero.style.display='none';main.style.display='none';}if(target==='timing')$('timingLab').style.display='none';if(target==='city')$('cityLab').style.display='none';}});
...
document.querySelectorAll('[data-city-tab]').forEach((button) => button.addEventListener('click', () => { document.querySelectorAll('[data-city-tab]').forEach((tab) => tab.classList.toggle('active', tab === button)); renderCity(button.dataset.cityTab); }));
document.querySelectorAll('[data-city-tab]').forEach((button) => { button.onclick = () => { document.querySelectorAll('[data-city-tab]').forEach((tab) => tab.classList.toggle('active', tab === button)); renderCity(button.dataset.cityTab); }; });
...
renderCity();
refreshCityContext();
document.querySelectorAll('.page-tabs,.city-tabs').forEach(el=>el.style.display='none');document.querySelector('.hero-grid').style.display='';document.querySelector('.main-stack').style.display='';$('timingLab').style.display='';$('cityLab').style.display='';
document.querySelector('.pricing-panel').style.display='';document.querySelector('.visual-grid').style.display='';
